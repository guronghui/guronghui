...
(() => {
  ...
  document.getElementById("quckly_register").style.background =
    "#fff url('" + c2.toDataURL() + "') no-repeat";
})();
  ...
  prompt.each(function (i) {
    ...
    if (i == prompt.length - 1) {
      if (upwd1 == upwd2) {
        ...
        $("#quckly_register p.upwd2_box .prompt")
          .html("密码不一致")
          .css("color", "#E4393c");
      }
    }
...
$("#f1>div").mousemove(function (e) {
  ...
  if (canMove) {
    ...
    var $last_a = $(
      "#f1>div>div>a:nth-child(" + (n == 1 ? $as.length : n - 1) + ")"
    );
    // console.log(n);
...
$("#f2>a.next").mouseup(function (e) {
  ...
  e.preventDefault();
  var $first_div = $("#f2>div>div:first-child");
  $first_div.css({ "margin-left": 0, transition: "margin-left 1s linear" });
