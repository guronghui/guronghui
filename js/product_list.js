     ...
     //console.log(pager);
	  ...
	  if(data.length==0){
		...
		for(i=0;i<data.length;i++){
	           html+=` 
            <li> 
           <a href="03_product_details.html?lid=${data[i].lid}"class="image"><img src="${data[i].pic}" alt=""></a>       
           <p class="price">￥${data[i].price}</p>

           <a href="javascript:;"  class="title">
			   <span>${data[i].title.split(/\d*.\d*英寸/)[0]}
		        </span>${data[i].title.match(/\d*.\d*英寸/)[0]}
		        <span>${data[i].title.split(/\d*.\d*英寸/)[1].split(/笔记本/)[0]}笔记本</span>
...
//加载分页效果	
  var pages=`<a class=${pager.pno==1?"disabled":""}>上一页</a>`;
	   ...
	   pages+=` <a data-page="${i}" class=${pager.pno==i?"active":""} >${i}</a> `
	   }
       pages+=`<a class=${pager.pno==pager.pageCount?"disabled":""}>下一页</a>`;
     $("#product_page").html(pages);
   $("#product_page>a:not(:last-child):not(:first-child)").click(function(e){
	   e.preventDefault();
	  // console.log(1321);
...
**************************************/
	...
	var  $top_end=$("div.aside_right li>a.cart").offset().top-$(window).scrollTop();
	...
	setTimeout(()=>{
	   ...
	   "position":"fixed",
		"top":$top_end+"px",
		"right":"0px",  
		"transition":"all "+$right_start/$(window).width()+"s  linear"
	  },10);
	setTimeout(()=>{
	   $imgClone.remove();
...
$order_a.click(function(e){
    ...
    $order_span.css("background","url(img/product_list/logo/sprite-arrow.png) no-repeat 0 -97px"); 
    $(this).find("span").css("background","url(img/product_list/logo/sprite-arrow.png) no-repeat 0 -115px"); 
